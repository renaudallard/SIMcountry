...
v0.2.3: native Rust daemon replacing the original Kotlin daemon (Android 16 broke `app_process` for shell uid), JSON-over-TCP control socket on `127.0.0.1:39351` with an APK-hash auth handshake, libbinder_ndk client for `ISub` / `IPhoneSubInfo` / `IEuiccController`, an ISO-keyed rule schema with an E.212 country picker and a Default SIMs card, plus a Wireless-ADB autostart flow that auto-recovers when Wi-Fi or Wireless Debugging come back online and surfaces an offline notification linking to Developer options when the daemon is down. Verified end-to-end on Motorola Lhotse / Android 16 (default-data sub-id switching, autostart over Wireless ADB, post-update auto-restart). Unit tests cover the rule matcher, the country-watcher state machine, the ADB protocol codec, the RSA legacy key serializer, Ed25519 group laws (including off-curve point rejection), the AES-GCM stream layer, and SPAKE2 client/server round trips: 48 tests in total.
